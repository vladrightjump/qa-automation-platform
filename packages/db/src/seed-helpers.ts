...
// Re-export the pure RNG + row-builder so existing call sites
// (`@qa/db` consumers) keep their imports stable. The actual
// definitions live in `bulk-seed-rng.ts` so unit + mutation tests can
// target them without the Prisma-touching code dragging coverage down.
export {
...
import { buildBulkProductRows } from './bulk-seed-rng';
...
export async function seedBulkProducts(
  ...
  const rows = buildBulkProductRows(count, rngSeed);
